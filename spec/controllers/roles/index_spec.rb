...
describe RolesController do
  ...
  describe "INDEX" do
    ...
    it "should return a 403 if the X-API-Token doesn't yield GET authorisation for Roles" do
    ...
    it "should accept match and search parameters" do
      Role.should_receive(:index).with(anything, nil, 'ue').and_return([])
      get :index, app: 'foo', search: 'ue'
      response.status.should == 200
    end
    
    it "should accept a group parameter" do
      Role.should_receive(:index).with(anything, 'name', nil).and_return([])
